...
const PlayerRowScene: PackedScene = preload(
	"res://src/ui_components/player_list/player_row/player_row.tscn"
)
...
func _sort_attributes(key: String) -> void:
	players.sort_custom(
		func(a: Player, b: Player) -> bool: return (
			(a.attributes.get(info_type) as Resource).get(key)
			< (b.attributes.get(info_type) as Resource).get(key)
		)
	)
