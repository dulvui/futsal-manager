...
const VisualPlayer:PackedScene = preload("res://src/match_simulator/visual_action/actors/player/visual_player.tscn")
...
func _player_setup() -> void:
	...
	for player in home_team.get_field_players():
	...
	for player in away_team.get_field_players():
		var visual_player:Node2D = VisualPlayer.instantiate()
