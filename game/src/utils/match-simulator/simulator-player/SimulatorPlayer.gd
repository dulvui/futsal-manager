...
func set_up(new_player,field_pos):
	...
	player = new_player
	if player["home"]:
		sector_pos = (field_pos+1) * 100
	else:
		sector_pos = 1200 -( (field_pos+1) * 100)
	current_sector = sector_pos/200
	start_pos = sector_pos
	
	min_sector = sector_pos - 200
	if min_sector < 0:
		min_sector = 0
	max_sector = sector_pos + 600
	if max_sector > 1200:
		max_sector = 1200
...
func make_offensive_with_ball_decision():
	...
	var sum: int = shoot_factor + pass_factor + move_up_factor + wait_factor
...
#	print(player["surname"] + " team has ball")
	...
	var wait_factor = randi()%8 # make depending on player stats and tema mentality
	
	var sum = move_up_factor  + wait_factor
	var decision_factor = randi()%sum
...
#	print(player["surname"] + " defends")
	...
	var decision_factor = randi()%(move_down_factor + wait_factor)
	
	if decision_factor < move_down_factor:
#		print("MOVES DOWN")
		emit_signal("move_down",player)
		move_up()
	else:
...
func check_pass():
	...
	#check mentality if more ofensive pass or possesion
	#check sector with best value
	# value gets calculated by near to goal + players in sector + opponents in sector
	# save best pass option, then can be used by pass
#	var sector_values = []
