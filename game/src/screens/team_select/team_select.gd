...
func _ready() -> void:
	...
	for continent: Continent in Config.save_states.temp_world.continents:
	...
	set_teams()
	var first_league: League = Config.save_states.temp_world.get_all_leagues()[0]
...
func set_teams(p_nation: Nation = Config.save_states.temp_world.get_all_nations()[0]) -> void:
...
func _on_select_team_pressed() -> void:
	active_team.staff.manager = Config.save_states.temp_manager
