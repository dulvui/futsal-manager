...
func from_json(json: Dictionary) -> void:
	...
	var json_resource_class_list: Array = global_class_list.filter(
	...
	while not json_resource_class_list_copy.is_empty():
		json_resource_class_list_copy = global_class_list.filter(
			func(d: Dictionary) -> bool:
				return str(d.path).begins_with("res://") and d.base in json_resource_class_list_copy
		).map(func(d: Dictionary) -> String: return d.class)
